...
<script lang="ts">
  ...
  import {dropDownMenu, showEditPlayListModal, guid} from "@/utils/utils"
  ...
  export default class PlayListView extends SelectContainer {
    ...
    onDistPlayListContentChanged(val: Array<File>) {
      this.$store.commit("playList/setPlayListContent", {
        ...
        content: val.map((o: File) => new PlayListContentDataItem(o.title, o.p, o.album))
      })
    }
    ...
    public mounted() {
      ...
      const onSort = (e: any) => {
        if (e.data.over && e.data.over.className.indexOf("token") !== -1) {
          e.cancel()
        }
      }
    ...
    rename() {
      showEditPlayListModal({isRename: true, oldName: this.distPlayList!.title}).then(name => {
